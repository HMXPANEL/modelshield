...
def submit_utr(req: SubmitUTRRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # FIX: block resubmission on all non-awaiting states
    ...
    # FIX: check UTR uniqueness across all payments to prevent fraud
...
# FIX: typed Pydantic model instead of raw dict — prevents crashes on missing/bad input
...
def admin_toggle_model(model_id: int, data: dict, admin: User = Depends(get_admin_user), db: Session = Depends(get_db)):
    ...
    if "cost_per_token" in data:
        model.cost_per_token = float(data["cost_per_token"])
    if "free_access" in data:
        model.free_access = bool(data["free_access"])
    db.commit()
...
def admin_add_provider_key(data: ProviderKeyCreate, admin: User = Depends(get_admin_user), db: Session = Depends(get_db)):
    pk = ProviderKey(provider=data.provider, api_key=data.api_key)
    db.add(pk)
    db.commit()
