...
# FIX: Engine and session are created lazily so DATABASE_URL env var
# can be set before first import without timing issues.
# This also makes it easy to swap to PostgreSQL in production.
_engine = None
_SessionLocal = None
...
# FIX: Use DeclarativeBase (SQLAlchemy 2.0) instead of deprecated declarative_base()
...
class Payment(Base):
    ...
    utr = Column(String, nullable=True, unique=True)  # FIX: unique prevents UTR reuse fraud
