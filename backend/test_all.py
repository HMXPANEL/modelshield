...
with TestClient(main.app) as client:
    ...
    # ─── [6] Admin ───────────────────────────────────────────────
    print("\n[6] Admin")
    # BUG FIX: admin email is now consistently "admin@modelshield.dev"
    ...
    check("Admin: analytics has all fields",
          r.status_code == 200 and all(k in r.json() for k in ["total_users", "total_tokens", "total_revenue", "total_requests"]))
    ...
    pk_list = client.get("/admin/provider-keys", headers=ah).json()
    check("Admin: provider keys list has entry", len(pk_list) >= 1)
    pk_id = pk_list[-1]["id"]
    ...
    r = client.post("/v1/chat/completions",
                    json={"model": "llama-3.1-8b-instant", "messages": [{"role": "user", "content": "hi"}]},
                    headers={"Authorization": "Bearer ms_fakefakefakefakefakefakefakefake1234567"})
