// BUG FIX: Dynamic API_BASE so frontend works on both localhost (Termux) and Render.com
// On Render, the frontend is served from the same origin as the API, so we use window.location.origin
// On localhost we need to point at the backend port (8000)
...
// ── Playground key storage ────────────────────────────────────────────────────
// Store newly created API keys in sessionStorage so the playground can use them
// without re-entering. Key: "pg_key_<id>", Value: raw API key.
function storeKeyForPlayground(id, rawKey) {
  try { sessionStorage.setItem("pg_key_" + id, rawKey); } catch {}
}

function getStoredPlaygroundKey(id) {
  try { return sessionStorage.getItem("pg_key_" + id); } catch { return null; }
}
